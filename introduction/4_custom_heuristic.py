...
def custom_heuristic(file_path):
    ...
    for passenger_index, passenger in df.iterrows():
        if passenger['Sex'] == "female" or (passenger['Pclass'] in (1, 2) and passenger['Age'] < 16):
...
def calculate_heuristic(file_path):
    predictions, df = custom_heuristic(file_path)
    ...
    print float(right) / len(predictions) * 100


if __name__ == "__main__":
    calculate_heuristic('titanic_data.csv')